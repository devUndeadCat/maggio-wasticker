...
const addMetadata = (packname, author) => new Promise(async(resolve, reject) => {
 ...
 try {
 let isMetadata = await fs.stat(`./metadata.exif`);
 if (isMetadata) await fs.unlink(`./metadata.exif`)
 } catch(err) {
   console.log("no metadata file to unlink")
 }
...
export const setExif = (path, author, packname) => new Promise(async(resolve, reject) => {
  try {
